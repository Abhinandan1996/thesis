...
def psi(u):
    return 0.5 * (lmbda + mu) * (0.5 * (tr(epsilon(u)) + abs(tr(epsilon(u))))) ** 2 + mu * inner(
        dev(epsilon(u)), dev(epsilon(u))
    )
...
E_phi = (
    Gc * l * inner(grad(p), grad(q))
    + ((Gc / l) + 2.0 * H(uold, unew, Hold)) * inner(p, q)
    - 2.0 * H(uold, unew, Hold) * q
) * dx
...
deltaT = 0.1
tol = 1e-3
conc_f = File("./ResultsDir2/phi.pvd")
fname = open("ForcevsDisp2.txt", "w")
